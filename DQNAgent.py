        ...
        while True:
            ...
            if is_train:
                ...
                prob = max(config.eps_start - (
                           ...
                           config.eps_end)
                print("game round: %d, live_time: %d, iteration: %d, loss: %.4f, action: %s, current game score: %d, prob %f" % (game_round, live_time, count, loss, str(action), self.game_agent.score, prob))
